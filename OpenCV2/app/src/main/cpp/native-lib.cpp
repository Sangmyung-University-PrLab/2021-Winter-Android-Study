...
extern "C"
JNIEXPORT void JNICALL
Java_ac_kr_smu_ImageProcessing_convert_1YUV420_1to_1ARGB8888(JNIEnv *env, jclass clazz,
                                                             jbyteArray y, jbyteArray u,
                                                             jbyteArray v, jintArray output_addr,
                                                             jint width, jint height,
                                                             jint y_row_stride, jint uv_row_stride,
                                                             jint uv_pixel_stride) {
    jbyte *yData = (*env).GetByteArrayElements(y, 0);
    jbyte *uData = (*env).GetByteArrayElements(u, 0);
    jbyte *vData = (*env).GetByteArrayElements(v, 0);
    jint *output = (*env).GetIntArrayElements(output_addr, 0);
    jint *out = output;
...
JNIEXPORT void JNICALL
Java_ac_kr_smu_ImageProcessing_processing(JNIEnv *env, jclass clazz, jlong input_addr,
                                          jlong output_addr) {
